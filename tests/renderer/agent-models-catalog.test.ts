...
describe('advanceAgentModelsPoll', () => {
	/** Drives the poll across a sequence of catalogs, returning each interval. */
	function run(sequence: readonly (AgentModelCatalog | undefined)[]): {
		intervals: (number | false)[];
		state: AgentModelsPollState;
	} {
		let state = initialAgentModelsPollState();
		const intervals: (number | false)[] = [];
		for (const data of sequence) {
			const result = advanceAgentModelsPoll(data, state);
