...
describe('agent-control AWARENESS parity', () => {
	...
	it('appends every rendered brief block, in the order the app joins them', () => {
		expect(readExtensionSource()).toMatch(
			/event\.systemPrompt,\s*playbook,\s*nudge,\s*planRefinement,\s*afkDirective,\s*afkWorkflowDirective,\s*delegationDirective,\s*languageDirective,\s*issueDirective,\s*coAuthorDirective,/,
		);
	});
