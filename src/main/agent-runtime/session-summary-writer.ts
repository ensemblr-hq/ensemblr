...
export interface WriteSessionSummaryInput {
	...
	fileBaseName?: string;
	/**
	 * Runtime-native session id of the agent process this record came from
	 * (`agent_sessions.runtime_session_id`), or null when the tab never opened
	 * one. Not the Ensemblr `agent_sessions.id` a chat tab's `agentSessionId`
	 * points at; the summary identifies its tab by `chatTabId` instead.
	 */
...
const STUB_BODY = '_Empty tab — no agent session was opened._\n';
...
async function runWriteSummary({
	...
	if (input.runtimeSessionId === null || transcriptEvents.length === 0) {
...
function renderFrontmatter(input: RenderFrontmatterInput): string {
	const lines = [
		...
		`runtimeSessionId: ${yamlNullable(input.runtimeSessionId)}`,
 ...
 */
function countTurns(events: readonly AgentSessionEventWire[]): number {
 ...
 */
function renderTranscript(events: readonly AgentSessionEventWire[]): string {
	return events
		.map((event) => {
 ...
 */
function extractText(event: AgentSessionEventWire): string {
	const payload = event.payload;
	if (payload?.kind !== 'message') {
		return '';
	}
