 ...
 * nothing about the control ports, the Pi session service, or the app-settings
...
import { getMainBranchForSession } from '../../storage/repositories/pi-session-repository.ts';
...
export function readSessionBriefNaming({
	...
	try {
		...
		const tab = getChatTabByPiSessionId({
			database,
			piSessionId: caller.sessionId,
		});
