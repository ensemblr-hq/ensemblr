...
/** Options for {@link createPiCliRpcAdapter}. */
export interface CreatePiCliRpcAdapterOptions {
...
/**
 * Builds the production `AgentAdapter` that drives `pi --mode rpc`. This
 * adapter owns every Pi CLI flag: it maps the provider-neutral session request
 * onto argv and writes the result back onto `metadata.args` before spawning.
 *
 * Lifecycle:
 ...
 */
export function createPiCliRpcAdapter(
	options: CreatePiCliRpcAdapterOptions = {},
): AgentAdapter {
...
/**
 * Appends Pi's per-session selection flags to the base RPC args: model
 * (`--model`), thinking level (`--thinking`), and the runtime-native session id
 * (`--session-id`, never the Ensemblr `agent_sessions.id`). Model and thinking
 * bind at spawn; mid-session changes go through the
 * `set_model`/`set_thinking_level` RPC commands instead.
 * @param input - Base argv plus the session's model, thinking, and runtime session id.
 * @returns The complete argv for this session's `pi` child.
 */
function buildPiSessionArgs({
	baseArgs,
	modelOverride,
	runtimeSessionId,
	thinkingLevel,
	...
	if (runtimeSessionId) {
		args.push('--session-id', runtimeSessionId);
	}
	return args;
}
...
interface CliRpcSession {
	publicSession: AgentAdapterSession;
}
...
function createCliRpcSession({
	...
	let closed = false;
	let pendingShutdownReason: AgentShutdownReason | null = null;
	 ...
	 */
	const finalizeShutdown = (reason: AgentShutdownReason): void => {
		if (closed) {
			return;
		}
		closed = true;
	...
	const getState = async (): Promise<AgentSessionState> => {
		if (closed || !child.stdin.writable) {
			throw new Error('Pi RPC session is not writable.');
		}
		const id = turnIdFactory();
	...
	const publicSession: AgentAdapterSession = {
		abort,
		close,
		getMetadata: () => metadata,
		getState,
