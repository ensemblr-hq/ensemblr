...
- After moving main-process files or imports, run `nub run typecheck`.
- Run the narrow `nub run test:<concern>` script for any changed behavior under a main concern.
- Run `nub run check` before finishing JavaScript, TypeScript, CSS, or JSON changes.
