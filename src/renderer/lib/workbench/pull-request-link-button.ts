...
/**
 * Builds the shared PR/deployment pill styling so both controls keep matching
 * borders. Every tinted tone draws its border at the same alpha, so no status
 * shouts louder than the rest of the header.
 *
 * Each tinted tone repeats its border under `dark:`. The pills render as a
 * `variant='outline'` button, whose `dark:border-input` outranks an unprefixed
 * border class on specificity — dropping the duplicate turns every border
...
export function getPullRequestLinkButtonClassName(tone: PullRequestHeaderTone) {
	return cn(
		...
		tone === 'pending' &&
			'border-status-warning/35 bg-background/90 text-foreground hover:bg-status-warning/15 dark:border-status-warning/35 dark:bg-background/80 dark:text-status-warning dark:hover:bg-status-warning/20',
		tone === 'blocked' &&
			'border-status-danger/35 bg-status-danger/10 text-status-danger hover:bg-status-danger/15 dark:border-status-danger/35',
		tone === 'merged' &&
			'border-pr-merged/35 bg-background/90 text-pr-merged hover:bg-pr-merged-soft dark:border-pr-merged/35 dark:bg-background/80 dark:hover:bg-pr-merged-soft',
		tone === 'neutral' &&
