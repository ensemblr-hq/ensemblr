...
import type { PiNoticeMetadata } from '@/renderer/types/pi-timeline';
import type {
	PiPersistedEnvelope,
	PiSessionEventWire,
} from '@/shared/ipc/contracts/pi-session';
import { describesGracefulTermination } from '@/shared/pi-rpc/process-exit';
...
export function buildInterruptedMessage(
	event: PiSessionEventWire,
	envelope: Extract<PiPersistedEnvelope, { kind: 'shutdown' }>,
): UIMessage | null {
	...
	return {
		id: `pi-event:${event.id}`,
		metadata: { notice: 'interrupted' } satisfies PiNoticeMetadata,
