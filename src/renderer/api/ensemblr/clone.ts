...
export function selectCloneDestination(): Promise<CloneDestinationSelectionResult> {
	return profileElectronIpcCall(
		{ channel: 'ensemblr:select-clone-destination', usesDatabase: false },
		() => getEnsemblrApi().selectCloneDestination(),
	);
}
...
export function prepareCloneGithubRepository(
	...
	return profileElectronIpcCall(
		{
			channel: 'ensemblr:clone-github-repository:prepare',
			usesDatabase: false,
		},
		() => getEnsemblrApi().prepareCloneGithubRepository(request),
	);
}
...
export function startCloneGithubRepository(
	...
	return profileElectronIpcCall(
		{ channel: 'ensemblr:clone-github-repository:start', usesDatabase: true },
		() => getEnsemblrApi().startCloneGithubRepository(request),
	);
}
