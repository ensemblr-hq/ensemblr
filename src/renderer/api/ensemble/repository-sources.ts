	...
	return queryOptions({
		queryFn: (): Promise<ListRepositoryBranchesResult> =>
			profileElectronIpcCall(
				{ channel: 'ensemble:list-repository-branches', usesDatabase: true },
				() => getEnsembleApi().listRepositoryBranches({ repositoryId }),
			),
		queryKey: ensembleQueryKeys.repositoryBranches(repositoryId),
...
export function repositoryPullRequestsQuery(repositoryId: string) {
	return queryOptions({
		queryFn: (): Promise<ListRepositoryPullRequestsResult> =>
			profileElectronIpcCall(
				{
					channel: 'ensemble:list-repository-pull-requests',
					usesDatabase: true,
				},
				() => getEnsembleApi().listRepositoryPullRequests({ repositoryId }),
			),
		queryKey: ensembleQueryKeys.repositoryPullRequests(repositoryId),
		staleTime: SOURCES_STALE_MS,
	});
}
...
export function repositoryIssuesQuery(repositoryId: string) {
	return queryOptions({
		queryFn: (): Promise<ListRepositoryIssuesResult> =>
			profileElectronIpcCall(
				{ channel: 'ensemble:list-repository-issues', usesDatabase: true },
				() => getEnsembleApi().listRepositoryIssues({ repositoryId }),
			),
		queryKey: ensembleQueryKeys.repositoryIssues(repositoryId),
		staleTime: SOURCES_STALE_MS,
	});
}
