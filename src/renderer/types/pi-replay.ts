/**
 * Timeline model produced by the Pi RPC replay reducer in `lib/pi-replay`, the
 * dev-only surface behind `/debug/pi-replay`. It folds raw Pi RPC frames from
 * `tests/fixtures/pi-captures/` into renderable items: streaming deltas grow a
 * single item, tool calls carry a lifecycle status, consecutive tool calls
 * group, and noise frames never become items (they only update
 * `PiTimelineSessionMeta`). See `docs/pi/event-taxonomy.md`.
 *
 * This is not the live conversation model — that one is provider-neutral and
 * lives in `types/agent-timeline.ts`.
 */
...
/** One user prompt echoed back by the agent. */
...
/** Any item the replay reducer can place in its ordered timeline. */
...
/** Timestamped reducer input: one parsed Pi RPC frame plus its arrival time. */
