...
import { getUnavailableWorkspaceIds } from '@/renderer/state/workspace';
...
export function useNavigateToLastUnread(): (
	...
	return useCallback(
		async (target: UnreadChatRef) => {
			...
			// A workspace mid-teardown is losing the worktree this chat lives in, so
			...
			if (getUnavailableWorkspaceIds().has(target.workspaceId)) {
				return;
			}
