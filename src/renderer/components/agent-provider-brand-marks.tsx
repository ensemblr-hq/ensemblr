...
/**
 * Mark and tinted swatch for each agent runtime. Claude wears the vendor's own
 * brand colour, held as `--ensemblr-brand-claude` so the hex is named once
 * rather than inlined per utility; Pi borrows Ensemblr's accent since it is the
 * house runtime.
 *
 * Kept beside the components rather than with them so the component module
 * exports components only and Fast Refresh can preserve their state.
 */
export const AGENT_PROVIDER_BRAND: Record<
	AgentProviderId,
	{ Logo: (props: { className?: string }) => ReactElement; tint: string }
> = {
	claude: { Logo: ClaudeLogo, tint: 'bg-brand-claude/12 text-brand-claude' },
	pi: { Logo: PiLogo, tint: 'bg-accent-strong/12 text-accent-strong' },
};
