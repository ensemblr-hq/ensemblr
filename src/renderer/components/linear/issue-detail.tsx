...
/**
 * Linear issue detail body: a scrolling two-column layout under the command bar
 * the route renders. The issue reads down the left as title, description, and
 * thread, while its properties stay in a rail on the right where each one is
 * editable in place. Below the two-column breakpoint the rail moves under the
 * title, so the status of an issue is still the first thing after its name.
 */
