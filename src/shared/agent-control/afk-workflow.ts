 ...
 */
const DELEGATE_MECHANICS_ENSEMBLR = `Pay for a hand-off once. Before every spawn, call \`ensemblr_list_models\`: its live result is the source of permitted runtime destinations and the user's role preferences. Choose the task role first, prefer a model tagged for it, and name the role in the brief; untagged setups keep working as before, and a meaningful departure from configured tags is allowed when the brief says why. An explicit model may use only a runtime in \`allowedRuntimes\`; omitting it still inherits on your runtime.
