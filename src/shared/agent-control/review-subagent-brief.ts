 ...
 * three things have to be said before the review prompt: it is that
 * orchestrator's sub-agent and does its own reading, it is not the committer,
 * and a follow-up asking it to fix its own findings is expected rather than out
 * of scope.
 *
 * The first of those is why the reviewer is a child rather than a peer
 * ([ADR 0062](../../../docs/adr/0062-open-an-agent-requested-review-as-a-sub-agent.md)):
 * a peer costs one of the workspace's two co-tenancy slots, which an unattended
 * run spends on its first review and cannot get back while the reviewer is open.
 * What it gives up in exchange is fanning readers out over a wide diff, so the
 * bullets below point at the one op that reads a wide diff for it.
 *
...
export const REVIEW_SUBAGENT_BRIEF_HEADER = 'YOU ARE THIS WORKSPACE’S REVIEWER';
 ...
 * `ensemblr_notify_orchestrator`, and an instruction to "tell the orchestrator"
 * with no id is an instruction to go looking.
 ...
 */
export function buildReviewSubAgentDirective(spawnerSessionId: string): string {
	return `${REVIEW_SUBAGENT_BRIEF_HEADER} — another orchestrator working in this workspace asked the app for a review of its change, and this conversation is that review. The guidelines below are the same ones the user's own Review button runs.

You were opened as that orchestrator's sub-agent, so the reading is yours to do rather than to hand out: nested delegation is blocked here. \`ensemblr_get_workspace_diff\` is what makes a wide change readable anyway — call it with \`stat: true\` first for the file list, then read the diff whole or one file at a time with \`filePath\`.
...
- **Say what you touched.** Your last message is the whole of what the orchestrator reads, so it carries your findings, the paths you changed, and anything you decided on your own. When you cannot review at all until somebody answers — the diff is empty, the brief contradicts the change — \`ensemblr_notify_orchestrator\` wakes it rather than leaving it waiting out a turn you cannot finish.`;
}
