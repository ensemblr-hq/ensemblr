 ...
 * rendered only when the user has opted in.
 *
...
/** Why the line exists, so it reads as the user's choice rather than self-promotion. */
const RATIONALE = `The user turned this on in Settings → Git; it is their call, already made, and not something to raise with them again or to leave out because a commit felt too small. \`${ENSEMBLR_CO_AUTHOR_EMAIL}\` is a real GitHub account, which is what makes the credit land rather than render as text.`;
...
/**
 * Renders the co-author block, or null when the user has not opted in.
 * @param enabled - Whether the user turned the credit on.
