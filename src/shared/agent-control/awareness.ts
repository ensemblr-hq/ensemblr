 ...
 * this tool belongs to an explicit user request or the AFK workflow, not routine
 * attended verification. Off, the allowance still reads correctly — it is the
...
- **Settings > Integrations > Dictation — \`dictation\`:** \`enabled\` is boolean; \`baseUrl\` is the OpenAI-compatible API root; \`model\` is the transcription model string; \`language\` currently accepts only \`en\`. The API key is separate and inaccessible. An endpoint change directs audio and the stored credential to that endpoint: change it only to the endpoint the user intends, never as a workaround for authentication.
...
- **Settings > General:** the Ensemblr root-directory picker changes where managed repositories/workspaces live and reconciles the project list. Version, manual update and restart controls are system actions, not preference patches.
