/**
 * Provider-neutral classification of child-process exits. Every agent runtime
 * Ensemblr spawns is a CLI child, so the question "was this exit a requested
 * stop or a real failure?" belongs here rather than inside any one runtime's
 * wire protocol module.
 */

/** Signals that mean a child process was asked to stop, not that it broke. */
 ...
 * Agent launchers re-report SIGTERM as 143 when the app quits, so an exit with
...
/**
 * Reports whether a child's exit was a requested stop rather than a failure, so
 * shutdown noise never reaches the timeline. SIGKILL is deliberately absent: a
 * force-kill the adapter did not schedule (an OOM kill, say) is a real crash.
