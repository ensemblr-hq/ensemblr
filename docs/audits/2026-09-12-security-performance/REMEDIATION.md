...
| # | Disposition |
| --- | --- |
| **SH-03** | **Was blocked — needs a product decision; since closed.** Setting `GrantFileProtocolExtraPrivileges: false` was tried and measured: the renderer entry is an ES module, module scripts are always fetched in CORS mode, and an opaque `file:` origin therefore blocks every chunk — blank window. Closing it for real means serving the renderer from `app://`, which changes the origin and orphans the `localStorage` 19 renderer files use. The fuse was therefore *declared* `true` in `forge.config.ts` with that measurement, and `tests/main/forge-fuses.test.ts` pins the whole fuse set. **Closed afterwards, in two releases:** [ADR 0071](../../adr/0071-mirror-renderer-local-storage-into-sqlite.md) mirrored the renderer's `localStorage` into SQLite so the origin could move without orphaning it, and [ADR 0072](../../adr/0072-serve-the-packaged-renderer-from-its-own-origin.md) moved the renderer to `app://bundle` and flipped the fuse off. |
...
| **PM-04** | Fixed as the audit asked (Plan Mode refused `ensemblr_set_branch_name`), which required reversing three passages of `awareness.ts` that actively instructed a planning agent to make that call. **Reverted on 2026-09-13 by product decision**: those passages were the design, not an oversight — ADR 0050 has a planning session name the work first so the board does not sit on `poulenc` for the whole interview, and the branch the op moves is the one the workspace was cut with, behind `applyBranchSlug`'s own gate. The finding's premise (a branch rename is as visible as a tracked-file write) stands; the conclusion does not. |
