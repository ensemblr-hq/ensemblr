...
Piductor targets Conductor feature parity, including creating workspaces from Linear issues. The product also needs first-class Linear issue management: login, issue browsing, issue CRUD, comments, status updates, and workspace creation from selected issues.
...
Piductor already uses macOS Keychain for secrets and keeps SQLite for app metadata/cache.

## Decision

Piductor v1 will include first-class Linear integration using OAuth login.
...
- Create a Piductor workspace from a Linear issue.
...
- User selects a Linear issue from Piductor.
- Piductor creates a git worktree workspace from the configured branch source.
- Piductor stores Linear issue metadata in SQLite.
- Piductor starts a Pi session with the issue context available in the initial composer/timeline.
- Piductor can update Linear issue status according to user action or repository setting, but should not silently change status without clear UI.
...
Using a Linear MCP server could provide issue operations through agent tooling, but Piductor needs app-native issue browsing, selection, and workspace creation. MCP can be additive, not the app integration source of truth.
