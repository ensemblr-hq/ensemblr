...
Ensemblr already uses macOS Keychain for secrets and keeps SQLite for app metadata/cache.

## Decision

Ensemblr v1 will include first-class Linear integration using OAuth login.
...
- Create an Ensemblr workspace from a Linear issue.
...
- User selects a Linear issue from Ensemblr.
- Ensemblr creates a git worktree workspace from the configured branch source.
- Ensemblr stores Linear issue metadata in SQLite.
- Ensemblr starts a Pi session with the issue context available in the initial composer/timeline.
- Ensemblr can update Linear issue status according to user action or repository setting, but should not silently change status without clear UI.
...
Using a Linear MCP server could provide issue operations through agent tooling, but Ensemblr needs app-native issue browsing, selection, and workspace creation. MCP can be additive, not the app integration source of truth.
...
- Ensemblr needs OAuth callback handling, PKCE/state validation, token refresh, disconnect, and Keychain storage.
