...
- Checkpoint refs live in the repository's shared object store, so a snapshot is repository-scoped rather than workspace-scoped: every sibling worktree on the same clone can read the blobs a turn captured. The deduplication that makes checkpoints cheap is what buys that, so the two cannot be separated without giving each workspace its own object store. Treat a checkpoint as visible to anything with access to the clone, and do not capture a path the workspace would not commit.
