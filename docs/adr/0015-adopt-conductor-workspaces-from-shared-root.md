...
Piductor should support pointing at the same root directory as Conductor. Users should be able to start filesystem/git work in one app and continue it in the other where practical.

Conductor's documented and observable shared surface includes root subdirectories, git worktrees, `conductor.json`, `.worktreeinclude`, scripts, branches, and GitHub state. Conductor also has private app-local state in its own SQLite database, which Piductor should not treat as a compatibility contract.
...
Piductor will adopt existing Conductor-created repositories and workspaces from a shared root using filesystem and git metadata.
...
- `conductor.json`, `piductor.json`, and `.worktreeinclude` at repository roots.
...
- Auto-detect valid workspaces and add Piductor records for missing ones.
...
- Piductor can discover Conductor-created worktrees without depending on private app state.
