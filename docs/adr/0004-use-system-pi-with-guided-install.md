...
Piductor should preserve the user's existing Pi environment so skills, extensions, prompt templates, themes, packages, settings, credentials, model configuration, project `.pi` files, context files, and sessions work the same way they work in the Pi CLI.

That compatibility is easiest when Piductor launches the user's installed `pi` executable. The tradeoff is onboarding: new users may not already have Pi installed.
...
Under that superseded approach, startup or first-session setup would have detected whether `pi` was available and runnable. If Pi was missing, Piductor would have guided the user through installation instead of silently failing or bundling a separate runtime by default.
...
Bundling Pi would simplify first launch but risks version drift, packaging complexity, and weaker compatibility with the user's normal CLI setup. It would make Piductor feel like a separate Pi distribution rather than a controller around the user's Pi environment.

### Require users to install Pi manually

Requiring a manual install is simple for implementation but creates poor onboarding. Piductor should detect the missing dependency and guide the user through installation.

### Managed Pi runtime in app support

Piductor could install and update Pi inside its own app support directory. This may be useful later, especially for users who do not want global npm installs. It is deferred because it adds runtime update ownership and packaging complexity.
...
- Compatibility testing would have compared the detected executable in Piductor with manual `pi --mode rpc` execution from the same workspace.
