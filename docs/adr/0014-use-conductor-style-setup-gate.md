...
Piductor targets Conductor parity and uses a selected Pi-compatible CLI executable as the agent runtime. Piductor also requires a local SQLite database, a managed root directory, git, shell/process execution, GitHub CLI, Linear OAuth for Linear workflows, and Pi RPC readiness.

## Decision

Piductor will use a Conductor-style setup gate adapted for Pi CLI RPC.
...
- Piductor root directory exists or can be created.
...
This was accepted in ADR 0005 but superseded by ADR 0025. Piductor should validate the selected executable and RPC contract for v1, not SDK importability.

## Consequences

- Piductor's first-run experience closely matches Conductor's prerequisite model.
