...
- **It holds six ops nothing else does.** Listing every project, navigating the
  app to a workspace, cutting a new workspace off a project, searching its own
  memory, and reading and changing app settings. A workspace agent is withheld all
  six: it belongs to one project and cannot act on another, and app settings sit
