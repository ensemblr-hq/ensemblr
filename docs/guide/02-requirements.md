...
Ensemblr does not ask you to trust a checklist. It ships fifteen setup checks
that probe the machine directly, and this page is that list. Every row below is
...
Plus macOS on Apple silicon. Everything else on this page Ensemblr either
provides, creates, or treats as optional.
...
| 6 | Environment variables | Every variable you marked required is set | Open environment settings | only if you marked one required |
| 7 | Git executable | `git --version` succeeds | copy `xcode-select --install`; open <https://git-scm.com/download/mac> | yes |
| 8 | GitHub CLI installed | `gh --version` succeeds | open <https://cli.github.com/> | yes |
| 9 | GitHub CLI authenticated | `gh auth status --hostname github.com --active` succeeds | copy `gh auth login --hostname github.com` | yes |
| 10 | Pi executable | A Pi-compatible binary resolves | Select Pi executable | gated |
| 11 | Pi agent directory | Pi's agent directory resolves and is readable | Retry only | gated |
| 12 | Pi RPC startup | `pi --mode rpc` starts and emits a valid JSONL frame | Select Pi executable | gated |
| 13 | Pi provider and model readiness | `pi --list-models` returns providers and models | Open Pi provider settings | gated |
| 14 | Claude Code executable | A `claude` binary resolves | copy `curl -fsSL https://claude.ai/install.sh \| bash`; open <https://code.claude.com/docs/en/setup>; Select Claude Code executable | gated |
| 15 | Linear connection | A live Linear OAuth session | Open integration settings | no |

Checks 1–6 are about Ensemblr's own state. Ensemblr creates the config file,
...
| Linear | OAuth token in the macOS Keychain, under service `dev.ensemblr.app.secret-store`. |
