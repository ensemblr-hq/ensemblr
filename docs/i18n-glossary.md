...
| Drawn *(a diagram)* | Нарисовать | Σχεδίαση | What an agent does to the architecture diagram, and the verb the undrawn empty state asks for. Replaces the `Scan`/`Сканировать`/`Σάρωση` row this table carried while a scanner seeded the document: nothing derives a diagram any more, so a term implying the app produced one misdescribes every surface. The timestamp reads as a past participle (`нарисовано` / `σχεδιάστηκε`). Never `Обновить`/`Ανανέωση` (refresh), which the app already uses for re-fetching data it did not compute, and never `Сгенерировать`/`Δημιουργία` — the app generates nothing here. |
