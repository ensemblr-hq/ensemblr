...
| Repository folder | Папка репозитория | Φάκελος αποθετηρίου | The repository's own directory on disk, as the delete dialog offers to remove it. Deliberately `папка`/`φάκελος` — a folder the user thinks of in Finder — against the `каталог`/`κατάλογος` of `Root directory` (Ensemblr's managed structure) and the `директория` of `Linked directory`. All three appear within two sentences of each other on the delete surface, so the split is load-bearing rather than stylistic. |
...
| Automatic system sleep | Автоматический переход системы в сон | Αυτόματη αναστολή λειτουργίας | The OS power state blocked by Caffeinate while allowing the display to sleep. Keep platform-neutral: Ensemblr supports multiple desktop platforms and detection can be unavailable. |
| Project | Проект | Έργο | The registered local repo, as distinct from a workspace. |
| Project root | Корень проекта | Ρίζα έργου | The existing checkout directory registered by Open local project; distinct from Ensemblr's managed `Root directory`. |
| Worktree | Worktree | Worktree | Git term; untranslated. |
| On disk *(where a folder lives)* | На диске | Στον δίσκο | The location, as in "the folder stays on disk" or "delete it from disk". Distinct from `Reclaim disk` below, which is about *space* (`место`/`χώρος`) rather than the medium. Singular and articled in Greek (`στον δίσκο`), never `από τον σκληρό δίσκο`, which names the hardware. |
...
| Third-party CLI harnesses *(Experimental switch)* | Сторонние CLI-харнессы | Harnesses CLI τρίτων | The Experimental row that turns the whole harness feature on. Built on `Harness` above rather than coining a second noun: Russian declines the transliteration into the plural, Greek keeps the Latin term the row above fixed and pluralizes it English-style, since a Greek plural of an untransliterated noun reads as a typo. `CLI` stays Latin in both — it is the acronym the harnesses themselves are named by. |
| Agent | Агент | Πράκτορας | |
| Agents *(panel title)* | Агенты | Πράκτορες | The workspace sidebar panel that lists open and closed agent conversations. |
| Working *(agent status)* | Работает | Εργάζεται | Active agent state. The tool-preview fallback adds `...` while no tool call is active. |
| Last recorded *(context usage)* | Последняя запись | Τελευταία καταγραφή | Prefix for the most recent known context reading of a closed conversation. The tooltip expands it to `Последняя запись: {{used}} из {{max}} токенов` / `Τελευταία καταγραφή: {{used}} από {{max}} διακριτικά`. |
| Closed parent chat | Закрытый родительский чат | Κλειστή γονική συνομιλία | Placeholder ancestor retained in the Agents hierarchy so an open child keeps its lineage. Uses **Chat**, never **Orchestrator**. |
| Sub-agent | Суб-агент | Υποπράκτορας | An agent another agent delegated to — Claude Code's `Task` tool, and a conversation a *workspace* agent spawned. Both terms were already fixed by the control-tool copy; this row records them. **Only a child the caller owns takes this noun.** Everything else a control-tool row can act on takes `Chat` instead — a peer beside it, the Review conversation, a batch mixing the two, an id the catalogue no longer holds (`control-tool.*.chat.*`), and every conversation the Concierge spawns (`control-tool.*.concierge.*`) — so `Запустил чат`/`Ξεκίνησε μια συνομιλία` rather than `Запустил суб-агента`. See **Orchestrator** below for why the second noun went away. |
| Orchestrator | — | — | **App jargon, not product vocabulary: never translate it, and never put it on a surface.** A root agent that owns a whole task and may delegate — as against the **Sub-agent** above, which is a child it collects. The distinction is real in the code (`TimelineAgentRole`, the agent-facing `notifyOrchestrator` op) and stays there, but a timeline row is read by the user rather than by an agent, so every row that once named it now says `Chat`. The `control-tool.*.orchestrator.*` keys folded into `control-tool.*.chat.*`, whose wording they already duplicated word for word; `control-tool.notify-orchestrator` — a sub-agent reporting *up* — became `Notified the parent chat` / `Уведомил родительский чат` / `Ειδοποίησε τη γονική συνομιλία`, which keeps the direction without the jargon. Retiring `Оркестратор`/`Ενορχηστρωτής` also retired the article trick they leaned on (`the orchestrator` above you, `an orchestrator` beside you), and with it `Равноправный оркестратор` / `ισότιμος ενορχηστρωτής` for "peer orchestrator", already rejected as a 30-character compound in a folded row. |
...
| Exited with code *(shell process)* | Завершился с кодом | Τερματίστηκε με κωδικό | The precise state of a shell command that returned a non-zero process status. It deliberately replaces the generic `failed` headline without claiming the program crashed or the output was unusable; the destructive tone still communicates that the requested condition was not met. The numeric code stays verbatim. |
...
| AFK | AFK | AFK | Plan mode's opposite number: the composer chip that tells the agent the user is away, so it decides for itself instead of asking. **Borrowed, not translated**, in the same class as `commit` and `worktree` — Russian- and Greek-speaking developers use the acronym daily, and the chip sits in the composer's control row, which has no room for `Не за компьютером` or `Λειτουργία απουσίας`. The meaning is carried by the tooltip, which *is* translated (`Работать без вопросов` / `Εργασία χωρίς ερωτήσεις`) and names the behaviour rather than the user's absence — what the user is choosing is that the agent stops asking. Never `АФК` in Cyrillic: the Latin form is the one that reads as the borrowed term rather than as a typo. |
| Unattended *(tab state)* | Без присмотра | Χωρίς επίβλεψη | The accessible name of the AFK marker on a chat's tab, read aloud in place of the keyboard-off glyph and the away tint. Deliberately **not** the borrowed `AFK` of the row above: that term is borrowed because the composer chip is a control the user clicks in a column with no room, and an acronym announced alone into the middle of a tab name reads as noise rather than as a state. It also names the user's absence where the chip's tooltip names the behaviour, for the same reason — the chip reports a choice being made, this reports a run already going without anyone watching. |
| Planning *(tab state)* | Планирование | Σχεδιασμός | The accessible name of the Plan mode marker on a chat's tab, the sibling of the row above. Built on the `Plan mode` term already fixed above rather than coining a second one — both keep that row's root (`план`, `σχεδι-`) — and stated as the activity in progress rather than as the mode's name, because the tab reports what the chat is doing, not which toggle is down. |
| Architecture diagram | Схема архитектуры | Διάγραμμα αρχιτεκτονικής | The module map of a workspace, authored by an agent and committed at `.ensemblr/architecture.json`. Nothing derives it. Russian says `схема` rather than `диаграмма` — the latter reads as a chart of *quantities*, and this is a structural drawing. Greek keeps `διάγραμμα`, which carries both senses. The tab itself is shortened to `Архитектура` / `Αρχιτεκτονική`, because the tab strip is the tightest column in the app and the icon already says it is a drawing. |
| Module *(a diagram node)* | Модуль | Μονάδα | One directory in the architecture diagram. Deliberately not `пакет`/`πακέτο` (package) — the node is a folder, not a published unit — and not `компонент`/`στοιχείο`, which the app already spends on React components. |
| Drawn *(a diagram)* | Нарисовать | Σχεδίαση | What an agent does to the architecture diagram; the undrawn empty state uses the Russian infinitive `Нарисовать` and the Greek action noun `Σχεδίαση`. Replaces the `Scan`/`Сканировать`/`Σάρωση` row this table carried while a scanner seeded the document: nothing derives a diagram any more, so a term implying the app produced one misdescribes every surface. The Russian timestamp uses the past participle `нарисовано`; Greek uses the finite passive verb `σχεδιάστηκε`, distinct from the action noun `Σχεδίαση` in this row. Never `Обновить`/`Ανανέωση` (refresh), which the app already uses for re-fetching data it did not compute, and never `Сгенерировать`/`Δημιουργία` — the app generates nothing here. |
| Fit to view | Вписать в панель | Προσαρμογή στον πίνακα | The diagram-toolbar control that scales the whole drawing down until it fits. Both languages name the *panel* rather than a window (`окно`/`παράθυρο`), because the diagram is framed inside the conversation pane and never inside a window of its own; `Панель`/`Πίνακας` is the term already fixed for that surface above. |
| Reset to actual size | Исходный размер | Πραγματικό μέγεθος | The sibling control that returns the diagram to 1:1 at the origin. Says *size* rather than "reset the view" in both languages: what the user is picking is a zoom level they can name, and `Сбросить`/`Επαναφορά` (reset) reads as discarding work — here nothing about the diagram changes, only how much of it is on screen. |
| Pan *(a canvas)* | Двигать | Μετακίνηση | Dragging the architecture diagram around under a fixed viewport. Russian uses the imperative-adjacent `двигать` in the instruction rather than coining a noun — there is no established Russian UI noun for panning, and `панорамировать` is photography jargon. Greek `μετακίνηση` (movement) is the ordinary word and needs no gloss. Distinct from `Scrollback` above, which is a terminal's retained history rather than a gesture. |
| Key names in an instruction *(N, P, 0, Enter, Escape)* | N, P, 0, Enter, Escape | N, P, 0, Enter, Escape | Keys are named by the legend printed on them, so they stay Latin in every language — a translated `Ввод`/`Εισαγωγή` names no key the reader can find. The diagram's canvas hint is the surface this settles: node stepping is matched on the *physical* key (`KeyN`/`KeyP`) precisely so the Latin legend stays true under a Cyrillic or Greek layout. Modifier words spelled out in prose (`plus`, `minus`) do translate — `плюс и минус`, `συν και πλην` — because they name the symbol rather than a legend. |
...
| Raw execution *(extension tool disclosure)* | Необработанное выполнение | Ακατέργαστη εκτέλεση | The host-owned disclosure of the actual tool name, input, and output beneath an extension-owned presentation. Both translations preserve the distinction between unprocessed execution data and the extension's friendly summary. |
| Web content *(tool activity)* | Веб-контент | Περιεχόμενο ιστού | Content searched, fetched, checked, or read through `pi-web-access`. Russian uses the compact borrowed `веб`; Greek uses the ordinary `ιστού`. The package name and URLs remain untranslated. |
| Background task | Фоновая задача | Εργασία παρασκηνίου | A durable shell or agent process launched by `pi-background-tasks`. Distinct from the agent's own checklist **Task**: this one keeps running after the tool call returns. `Fusion` and `Pi` are product names and remain untranslated in activity titles. |
| AST *(code structure)* | AST | AST | The abstract syntax tree inspected by pi-lens structural tools. Keep the established developer acronym in all languages; translate only the action around it (`Поиск AST` / `Αναζήτηση AST`). |
| LSP navigation | Навигация LSP | Πλοήγηση LSP | Code navigation through a language server. `LSP` stays Latin as the protocol acronym; translate the operation around it. |
| MCP server / tool | MCP-сервер / MCP-инструмент | Διακομιστής MCP / Εργαλείο MCP | The Model Context Protocol gateway and capabilities surfaced by `pi-mcp-adapter`. `MCP` stays Latin; Russian hyphenates compounds, while Greek uses the acronym as a qualifier. |
| Context7 documentation | Документация Context7 | Τεκμηρίωση Context7 | Library documentation fetched by the Context7 extension. `Context7` is a product name and stays unchanged; the short timeline rows may say `Документация Context7` / `Τεκμηρίωση Context7`. |
| Effective config *(pi-lens)* | Действующая конфигурация | Ενεργή διαμόρφωση | The resolved configuration and provenance pi-lens reports after all config layers have been applied. Distinct from editing app settings: this row describes the configuration currently in force. |
| Cache hit | Найдено в кэше | Βρέθηκε στην κρυφή μνήμη | A tool result served from its local cache rather than fetched again. Use the completed-result phrase in compact previews rather than a literal noun. |
| Project overview *(code)* | Обзор проекта | Επισκόπηση έργου | The repository-wide dependency and risk overview returned by pi-lens `project_report`. Uses **Project** from the core vocabulary because the report spans the registered codebase rather than one workspace change; `overview` keeps it distinct from the module-level **Module outline** below. |
| Module outline *(code)* | Структура модуля | Δομή μονάδας | The compact structural overview returned by pi-lens `module_report`. Uses **Module** from the architecture vocabulary above because both name one code module; `outline` is the module's symbol structure, not an architecture diagram or a prose report. |
| Code symbol | Символ кода | Σύμβολο κώδικα | A named declaration such as a function, class, interface, or method, as read by pi-lens `read_symbol`. The compact tool row drops the qualifier and reads `Чтение символа` / `Ανάγνωση συμβόλου` because the file chip already establishes the code context; `symbol_search` uses the same noun in `Поиск символов` / `Αναζήτηση συμβόλων`. |
| Run code *(context-mode)* | Запуск кода | Εκτέλεση κώδικα | The `ctx_execute` activity row. Uses a noun phrase in Russian and Greek so it reads as completed activity after the call settles as well as an action while it runs; never translate the programming-language name or the source under its `Код:` / `Κώδικας:` label. |
| Process file *(context-mode)* | Обработка файла | Επεξεργασία αρχείου | The `ctx_execute_file` activity row. Names processing rather than reading because the submitted program derives an answer from the file and may do more than inspect its text; the pinned file chip carries the target. |
| Run commands *(context-mode)* | Запуск команд | Εκτέλεση εντολών | The `ctx_batch_execute` activity row. The collapsed preview names each submitted command by its caller-provided label; the expanded `Commands:` / `Команды:` / `Εντολές:` section preserves the raw shell programs for auditability. |
| Fetch and index *(context-mode)* | Загрузка и индексация | Λήψη και ευρετηρίαση | The `ctx_fetch_and_index` activity row. Names both operations because the tool retrieves remote documentation and stores it in the searchable knowledge base. |
| Index context *(context-mode)* | Индексация контекста | Ευρετηρίαση περιβάλλοντος | The `ctx_index` activity row. Uses **Context** in the model/tooling sense and pins a local target when one was supplied. |
| Search context *(context-mode)* | Поиск по контексту | Αναζήτηση στο περιβάλλον | The `ctx_search` activity row. Its preview carries the submitted search phrases; the expandable body carries only focused results. |
| Check context mode *(context-mode)* | Проверка context-mode | Έλεγχος context-mode | The `ctx_doctor` activity row. `context-mode` stays in its package spelling because the check diagnoses that installed tool. |
| Open Insight *(context-mode)* | Открытие Insight | Άνοιγμα Insight | The `ctx_insight` activity row. `Insight` is the hosted product name and remains untranslated. |
| Purge context *(context-mode)* | Удаление контекста | Εκκαθάριση περιβάλλοντος | The destructive `ctx_purge` activity row. Names permanent removal rather than the app's ordinary clear-chat action. |
| Context usage *(context-mode tool)* | Использование контекста | Χρήση περιβάλλοντος | The `ctx_stats` activity row. Reuses **Context usage** from the app's model-usage surfaces rather than coining a second term. |
| Upgrade context mode *(context-mode)* | Обновление context-mode | Αναβάθμιση context-mode | The `ctx_upgrade` activity row. `context-mode` keeps its package spelling; the output supplies the shell command that performs the upgrade. |
| Export *(code)* | Экспорт | Εξαγωγή | A declaration exposed by a module. Used only in the compact count beside a module outline; the noun is developer vocabulary in all three languages. |
...
| Linked directory | Прикреплённая директория | Συνδεδεμένος κατάλογος | An arbitrary folder on disk a chat's agent was given access to. It is a sticky chat-level reference, not a per-message attachment. Deliberately `директория`, not the `каталог` of `Root directory`: for a folder the user picked themselves, `каталог` reads dated in Russian. `Каталог` stays for the managed root, which is Ensemblr's own structure. Greek uses `κατάλογος` for both. Never `рабочее пространство`/`χώρος εργασίας` — that is `Workspace`. |
...
| Caffeinate | Не давать засыпать | Αποτροπή αναστολής λειτουργίας | Prevent automatic system sleep while agents work, on macOS and Linux; not display sleep or hibernation. Russian keeps the existing action label; Greek uses the OS term for suspend rather than `αδρανοποίηση` (hibernate). |
...
| GitHub organization | Организация | Οργανισμός | Takes the same nouns as `Organization *(Linear)*` above rather than coining a second pair — a user reading `Организация`/`Οργανισμός` in the quick-start owner picker is looking at the same kind of thing they see on a Linear account badge, and two words for it would suggest otherwise. The proper noun `GitHub` qualifies it in prose and stays English. |
| GitHub owner | Владелец на GitHub | Κάτοχος στο GitHub | Whose namespace a new repository is created under — the signed-in user or one of their organizations, which is the `OWNER` in GitHub's own `OWNER/REPO`. Deliberately **not** `Аккаунт`/`Λογαριασμός` from the row above, which names a *connected integration* Ensemblr holds credentials for; a GitHub organization the user cannot publish into is still listed here, and calling it an account would promise a sign-in that does not exist. The picker's placeholder does say `Выберите аккаунт`/`Επιλέξτε λογαριασμό`, because at the moment of choosing, what the reader is scanning is a list of accounts. |
...
| Checksum | Контрольная сумма | Άθροισμα ελέγχου | The SHA-256 digest GitHub publishes for a release asset, which the Linux updater proves a download against before swapping it in. Both languages take the established term and pair with `Checkpoint`'s `Контрольная точка`/`Σημείο ελέγχου` above rather than transliterating to `чексумма`/`τσεκ-σαμ`. `SHA-256` and `GitHub` stay English. |
...
| Delete | Удалить | Διαγραφή | Permanent and irreversible, against `Archive` above, which is the recoverable path. Perfective in Russian: this is the confirm button on a dialog that has already explained itself, not an ongoing action. Never `Убрать`/`Αφαίρεση`, which the catalogue uses for taking something off a surface while it survives elsewhere. |
