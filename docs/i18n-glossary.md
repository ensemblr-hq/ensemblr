...
| Sub-agent | Суб-агент | Υποπράκτορας | An agent another agent delegated to — Claude Code's `Task` tool, and a conversation a *workspace* agent spawned. Both terms were already fixed by the control-tool copy; this row records them. **Not what the Concierge spawns:** its children are root orchestrators the user can open and talk to, so the same control tools are titled with `Chat` on that surface (`control-tool.*.concierge.*`) — `Запустил чат`/`Ξεκίνησε μια συνομιλία` rather than `Запустил суб-агента`. **Nor what a peer or the Review conversation is** — see **Orchestrator** below. |
| Orchestrator | Оркестратор | Ενορχηστρωτής | A root agent that owns a whole task and may delegate — as against the **Sub-agent** above, which is a child it collects. Both terms were already fixed by `control-tool.notify-orchestrator`, where a sub-agent reports *up* to the one that spawned it; the `control-tool.*.orchestrator.*` keys are the other direction, an orchestrator acting *sideways* on a conversation it does not own. Both cases a workspace row can mean — a peer opened beside it, and the Review conversation `ensemblr_start_review` opens — are root orchestrators, and one noun covers them: the article carries the rest (`the orchestrator` above you, `an orchestrator` beside you), and they barely meet — `notifyOrchestrator` is refused to a root, and a sub-agent is blocked from `sendFollowUp`/`startConversation`/`waitForAgents`, so only the three conversation *read* ops can put both readings on one transcript. `Равноправный оркестратор` / `ισότιμος ενορχηστρωτής` for "peer orchestrator" was rejected — a 30-character compound in a folded row for a distinction the noun does not have to carry. |
