...
| Architecture diagram | Схема архитектуры | Διάγραμμα αρχιτεκτονικής | The live module map of a workspace: directories as nodes, cross-module imports as edges. Russian says `схема` rather than `диаграмма` — the latter reads as a chart of *quantities*, and this is a structural drawing. Greek keeps `διάγραμμα`, which carries both senses. The tab itself is shortened to `Архитектура` / `Αρχιτεκτονική`, because the tab strip is the tightest column in the app and the icon already says it is a drawing. |
...
| Scan *(the workspace)* | Сканировать | Σάρωση | What seeding the architecture diagram does: walk the source tree and derive the module graph. It runs once, when the workspace is created, so the only surface carrying the term is the timestamp, as a past participle (`просканировано` / `σαρώθηκε`) — there is no rescan control to word in the imperative. Never `Обновить`/`Ανανέωση` (refresh), which the app already uses for re-fetching data it did not compute. |
