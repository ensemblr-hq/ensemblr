...
Documentation for Ensemblr — a desktop orchestrator for multi-agent coding work,
...
The current build is [`0.1.21`](https://github.com/ensemblr-hq/ensemblr/releases/tag/v0.1.21), for macOS on Apple silicon and Intel and Linux on x86-64. On macOS, `brew install --cask ensemblr-hq/tap/ensemblr` or download the `.dmg` ([Apple silicon](https://github.com/ensemblr-hq/ensemblr/releases/download/v0.1.21/Ensemblr-0.1.21-arm64.dmg), [Intel](https://github.com/ensemblr-hq/ensemblr/releases/download/v0.1.21/Ensemblr-0.1.21-x64.dmg)) — signed, notarized and stapled. On Linux, `curl -fsSL https://www.ensemblr.dev/install.sh | sh`, or take the [`.AppImage`](https://github.com/ensemblr-hq/ensemblr/releases/download/v0.1.21/Ensemblr-0.1.21-x64.AppImage) yourself — it is unsigned, because Linux has no equivalent to notarization. You bring your own agent CLI — Pi or Claude Code, one is enough — plus `git` and an authenticated `gh`.
...
- [`adr/`](./adr) — **73** Architecture Decision Records, numbered `0001`–`0074` (`0007` was withdrawn before acceptance), covering the accepted decisions and their supersessions. ADRs are historical records: some cite planning documents that were removed before the public release, and those references are left as written rather than rewritten after the fact.
...
- [`../AGENTS.md`](../AGENTS.md) — contributor policies (Bun, Biome, Jotai, Tailwind, JSDoc). Each `src/*` subtree has its own scoped `AGENTS.md`.
