...
The current build is [`0.1.3`](https://github.com/ensemblr-hq/ensemblr/releases/tag/v0.1.3), for macOS on Apple silicon and Linux on x86-64. On macOS, `brew install --cask ensemblr-hq/tap/ensemblr` or [download the `.dmg`](https://github.com/ensemblr-hq/ensemblr/releases/download/v0.1.3/Ensemblr-0.1.3-arm64.dmg) — signed, notarized and stapled. On Linux, `curl -fsSL https://www.ensemblr.dev/install.sh | sh`, or take the [`.AppImage`](https://github.com/ensemblr-hq/ensemblr/releases/download/v0.1.3/Ensemblr-0.1.3-x64.AppImage) yourself — it is unsigned, because Linux has no equivalent to notarization. You bring your own agent CLI — Pi or Claude Code, one is enough — plus `git` and an authenticated `gh`.
