...
The current build is [`0.1.0-beta.5`](https://github.com/ensemblr-hq/ensemblr/releases/tag/v0.1.0-beta.5) — signed, notarized, Apple silicon only. [Download the `.dmg`](https://github.com/ensemblr-hq/ensemblr/releases/download/v0.1.0-beta.5/Ensemblr-0.1.0-beta.5-arm64.dmg). You bring your own agent CLI — Pi or Claude Code, one is enough — plus `git` and an authenticated `gh`.
...
- [`adr/`](./adr) — **51** Architecture Decision Records, numbered `0001`–`0052` (`0007` was withdrawn before acceptance), covering the accepted decisions and their supersessions. ADRs are historical records: some cite planning documents that were removed before the public release, and those references are left as written rather than rewritten after the fact.
