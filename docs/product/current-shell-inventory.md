...
| Setup diagnostics banner | Setup blockers keep the shell visible and explain remediation should happen in the setup dock. | Implemented behavior | `App.tsx` selects the setup dock and disables composer when diagnostics are not ready. |
...
| Dock tabs | Bottom-right Setup, Run, and Terminal tabs stay visible with review/timeline context. | Implemented shell behavior | Dock tab state is route-backed and the dock is collapsible/resizable. Process-backed content is future work. |
...
| Sidebar health footer | App health and readiness status remain visible in the shell. | Implemented behavior | Health uses TanStack Query over the typed preload bridge. |
