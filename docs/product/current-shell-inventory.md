...
implemented shell should be treated as the settled intended layout, and future
agents should not reopen settled shell decisions unless a direct product
...
| Chat/session tabs | Multiple Pi sessions per workspace with active tab, close, restore, and new-chat affordances. | Implemented shell behavior | The active session is a URL path param (`/chats/$chatId`) remembered per workspace, so switching workspaces restores the last open chat; renderer-local close/restore are implemented. Context-aware ⌘/Ctrl+W close action: in workspace view, closes the active tab (with smart behavior: close, no-op for empty sole tab, or reset sole chat tab); in Settings, returns to the previous screen; on other screens, falls back to closing the window. Since #264 this rides the menu command bus rather than a dedicated `close-action` state concern (now deleted): the surface registers a `tab.close` handler through `src/renderer/state/menu-commands/`, and the menu item stays enabled with nothing registered so ⌘W falls through to closing the window instead of being swallowed. See `src/renderer/state/menu-commands/` and `src/renderer/state/workspace/session-tab-close.ts`. Session tabs are drag-reorderable (motion/react `Reorder.Group`, axis x, enabled when more than one tab) with order persisted to SQLite via the `reorderChatTabs` IPC contract and a `position` column (not a Jotai atom); see `conversation-panel/session-tabs.tsx`, `state/workspace/session-tabs.ts`, and `main/storage/repositories/chat-tab-repository.ts`. New chat remains future Pi/session work; there is no open-chat-tab cap (the five-tab limit was removed by ADR 0039). |
...
| Composer | Rich draft editor with inline attachment chips, model/thinking badges, setup-disabled reason, and send controls. | Implemented behavior | Prompt submit and stop are wired to both runtimes; drafts are per-chat-tab Jotai atom families keyed by chatId (`state/composer/composer-drafts.ts`), and `ComposerPanel` keys its own body so a chat switch cannot re-seed the previous tab's draft. **The text area is a Lexical plain-text editor** (`conversation-panel/composer/editor/`) whose chips are decorator nodes; it publishes the draft out as text plus runs and chips in document order, so the outgoing prompt interleaves each `<attached_file>` block where its chip sat. The three parallel upload/mention/external atom families collapsed into **one ordered `ComposerAttachment` list** (#256) covering files, pasted images and long text, `@`-mentions, Linear/GitHub issues, review-comment threads, and linked directories. Payloads persist to a content-addressed store under `.context/attachments/<digest>/`; issues and comment threads are serialized to whole markdown documents rather than summary lines. Mid-turn sends land in a per-tab **follow-up queue** pinned above the composer (#258, #260), and a cross-workspace "Last unread" pill sits in the controls row (#255). The model/thinking picker is live, and the slash catalogue is seeded from a versioned localStorage LRU and ranked by pick history (#241). Setup diagnostics can still disable the composer. |
