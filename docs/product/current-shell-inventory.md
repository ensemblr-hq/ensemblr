...
| Workspace context menu | Mark unread, pin, set status, rename, and archive actions. | Visual placeholder for planned behavior | Pin and archive are implemented; archive is now durable via the real `archiveWorkspace` mutation. Mark unread, rename, and status changes have visible affordances but no durable behavior. The status target needs confirmation before implementation. |
...
| Checks tab | PR title/description, git status, checks, comments, todos, no-PR state, and ready-to-merge flow. | Implemented behavior | Sections and state shapes are visible. Live `gh` metadata and polling landed (PR/statuses/deployments/review-threads via `github-service.ts`), and todos are wired (`checks-panel/checks-panel.tsx` `useTodoActions`, `workspace.pullRequest.todos`). Add-comment-to-Pi-context and a merge confirmation dialog remain future work. |
