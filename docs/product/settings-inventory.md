...
| `~/.config/piductor/config.json` | Declarative user defaults, managed policy-like settings, keybinding/UI defaults, repository matching rules. |
| Repository config | Shared project behavior in `piductor.json`, with `conductor.json` compatibility. Use for scripts, run mode, files-to-copy, and team-shared repository defaults. |
| Pi user environment | Pi auth, models, provider settings, skills, extensions, prompts, themes, sessions, and project `.pi` resources. Piductor should not duplicate this as source of truth. |
...
| Setting | Conductor mapping | Piductor adaptation | Storage |
| --- | --- | --- | --- |
...
| Completion sound | Direct. | Same, using Piductor sound assets. | SQLite with optional config default. |
| Auto-convert long pasted text | Direct. | Same, producing Piductor/Pi attachments. | SQLite with optional config default. |
...
| Personality/style | Provider-specific. | Pi system/personality setting if supported; otherwise Piductor prompt preset. | SQLite/config; repo overrides possible. |
| Default plan mode | Direct concept. | Pi planning mode if available; otherwise Piductor session mode. | SQLite/config; open SDK check. |
...
| Auth method | Direct concept. | Pi auth mechanism; likely inherited from Pi environment. | Pi environment; Piductor metadata in SQLite. |
...
| Setting | Conductor mapping | Piductor adaptation | Storage |
| --- | --- | --- | --- |
| Theme | Direct. | Piductor-specific themes. | SQLite/config. |
| Colored sidebar diffs | Direct. | Same. | SQLite/config. |
| Accessible colors | Direct. | Piductor accessibility palette variants. | SQLite/config. |
| Code theme | Direct. | Piductor code/diff highlighting theme. | SQLite/config. |
| Mono font | Direct. | Font for code, diffs, and inline code. | SQLite/config. |
| Code ligatures | Direct. | Same. | SQLite/config. |
| Markdown style | Direct. | Piductor markdown rendering preset. | SQLite/config. |
...
| Setting | Conductor mapping | Piductor adaptation | Storage |
| --- | --- | --- | --- |
| App account identity | Direct in Conductor. | Deferred for v1; Piductor is local-first. | Not applicable in v1. |
...
| Tool approvals | Direct concept. | Piductor permission mode mapped to Pi CLI/RPC tool allow/exclude controls where available. | SQLite/config; repo policy override possible. |
| Sign out | Direct if cloud account exists. | Deferred unless a future Piductor account exists. | Not applicable in v1. |
...
| Setting | Conductor mapping | Piductor adaptation | Storage |
| --- | --- | --- | --- |
| Root directory | Direct. | Piductor root with optional Conductor shared-root interoperability. | SQLite current value; config override/default. |
...
| Setting | Conductor mapping | Piductor adaptation | Storage |
| --- | --- | --- | --- |
| Repository record/name/icon | Direct. | Same, with Piductor icon choices. | SQLite. |
| Root path | Direct. | Path to managed or adopted repository. | SQLite only. |
| Workspaces path | Direct. | Path to workspaces under Piductor/shared root. | SQLite only. |
...
| Setting | Conductor mapping | Piductor adaptation | Storage |
| --- | --- | --- | --- |
| Preview URL template | Direct. | Support `PIDUCTOR_*`; support `CONDUCTOR_*` compatibility variables for Conductor-compatible repos or explicit opt-in. | SQLite personal override; shared default in `piductor.json` if added. |
...
| Create shared config file | Direct. | Create `piductor.json` first; support `conductor.json` for compatibility. | Repository config. |
...
| Setting | Conductor mapping | Piductor adaptation | Storage |
| --- | --- | --- | --- |
| Code review preferences | Direct. | Custom Pi instructions for review action. | SQLite personal override; `piductor.json` shared if safe. |
| Create PR preferences | Direct. | Custom Pi instructions for PR action. | SQLite personal override; `piductor.json` shared if safe. |
| Fix errors preferences | Direct. | Custom Pi instructions for fix-errors action. | SQLite personal override; `piductor.json` shared if safe. |
| Resolve conflicts preferences | Direct. | Custom Pi instructions for conflict resolution action. | SQLite personal override; `piductor.json` shared if safe. |
| Branch rename preferences | Direct. | Custom Pi instructions for deriving branch/workspace names. | SQLite personal override; `piductor.json` shared if safe. |
| General preferences | Direct. | Custom Pi instructions prepended to new chats in this repository. | SQLite personal override; `piductor.json` shared if safe. |
...
2. `piductor.json` at repository root.
3. `conductor.json` at repository root.
4. Built-in defaults.

For app-wide behavior, use:

1. Locked/managed settings from `~/.config/piductor/config.json`, if supported by schema.
2. User-selected settings in SQLite.
3. Declarative defaults from `~/.config/piductor/config.json`.
4. Built-in defaults.
