...
- `src/shared/` and `src/main/` return locale-neutral codes rather than English labels, so adding a code there is a user-facing change: the renderer mapper needs its `t()` case and that key needs `ru` and `el`. Two main-process catalogues are the exception, because they render their own copy with no renderer to hand a code to: the native menu table in `src/main/menu/menu-strings.ts` and the desktop-notification table in `src/main/agent-runtime/notification-strings.ts`. A new entry in either adds all three languages there.
