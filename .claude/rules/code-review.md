...
When the `code-review` skill is triggered, run automated diagnostics as its final step — after the functional review and after `nub run check` and `nub run typecheck`, and before reporting a change ready or opening a PR.
...
- These diagnostics are additive, not a replacement for `nub run check` and `nub run typecheck`. Run those first.
