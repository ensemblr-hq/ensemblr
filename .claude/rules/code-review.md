...
- Check translation completeness on the changed diff. Every catalogue key the diff adds must have a non-empty `ru` and `el` value, and a user-facing surface the diff touches must leave no hardcoded literal and no empty value behind — `bun run i18n:status` reports the per-locale numbers. See `.claude/rules/i18n.md`. Pre-existing gaps in files the diff does not touch are out of scope.
