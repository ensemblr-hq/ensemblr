 ...
 *
 * Run `bun run credits:generate` after any dependency change;
...
function readManifest(path) {
	try {
		...
		throw new Error(`Could not read package manifest ${path}.`, {
			cause: error,
		});
	}
}
...
/**
 * Returns direct third-party package names, excluding internal workspaces.
 * @param dependencies - Dependency names mapped to their declared ranges
 * @returns External package names in declaration order
 */
function externalDependencyNames(dependencies) {
...
export function buildCreditsManifest() {
	...
	return [
		...collect(externalDependencyNames(root.dependencies), 'runtime'),
		...collect(externalDependencyNames(root.devDependencies), 'development'),
	];
}
...
function renderModule(packages) {
	...
	return `// Generated by scripts/generate-credits.mjs — run \`bun run credits:generate\`.
// Do not edit by hand; tests/main/credits-manifest.test.ts fails on drift.

import type { CreditPackage } from './credits';

/** Every external direct dependency of Ensemblr, credited in the native About panel. */
