...
- **The updater selects by architecture.** Each release now carries one feed document per target, named `update-<platform>-<arch>.json`, and a build reads the one for its own platform and `process.arch` — an Intel install never moves to arm64, or the reverse. `update-darwin-arm64.json` keeps its exact name and shape, because already-installed Apple-silicon builds read it by that name and cannot be patched.

### Changed

- **The package manager is Bun again, and workspace setup is fast.** Ensemblr runs `bun ci` once per workspace and every workspace is its own git worktree, so each used to extract 845 packages into a fresh ~1.1 GB `node_modules`. Bun's global cache, cloned over APFS, turns that into metadata operations: `bun install` into an absent `node_modules` with a warm cache measured 7.1 s against the 26–40 s `npm ci` baseline, and `bun ci` warm 0.2 s. `bun.lock` replaces `package-lock.json` and stays at `lockfileVersion: 1` — dependabot-core's Bun parser raises on a higher one — with all 1250 packages on the versions the npm lockfile pinned (`bun pm migrate` under Bun 1.4.2, stamped back to 1, loaded under Bun 1.3.13). `bunfig.toml` pins the hoisted linker Forge's `PACKAGE_KEEP_*` filters need, `trustedDependencies` lists only `esbuild`, `fs-xattr`, and `macos-alias` (`node-pty` is deliberately excluded so Forge builds it against Electron's ABI), and `.npmrc` is gone. Node 24 remains the runtime. Supersedes [ADR 0038](./docs/adr/0038-migrate-package-manager-bun-to-npm.md); see [ADR 0073](./docs/adr/0073-move-the-package-manager-from-npm-to-bun.md).
- **`scripts/with-pinned-node.sh` is removed.** Ensemblr already injects the workspace directory's login-shell `PATH` — which activates mise and puts Node 24 and Bun on it — into setup scripts, run scripts, and terminals, so the wrapper was redundant for everything the app spawns. That injection is skipped whenever `[environment_variables]` defines `PATH` at all, so never set one. A plain non-login shell outside Ensemblr no longer self-corrects; it now fails loudly on `scripts/require-node-version.mjs`.
- **Starting a terminal no longer spends a delegation spawn.** `ensemblr_start_terminal` reserved a durable, lifetime, per-root-tree spawn slot that was refunded only when creation failed, so closing a terminal never gave it back and an orchestrator that opened and closed terminals across a long session was eventually refused everything with `Root-tree spawn quota of 20 exhausted`. A PTY spawns nothing and cannot recurse, so it is not delegation: terminals now take their own guard — 8 open per root tree, 10 starts per minute — with the open count derived from `listTerminals` on every attempt rather than from a ledger, so a close by the user returns the slot as reliably as a close by the agent. A dock terminal counts while its tab is listed, running or not; a script terminal counts only while it runs, so a restart does not accumulate. A start in flight holds its slot through `reserveTerminalStart` so concurrent siblings cannot all read the same pre-creation count and all pass, and `countOpen` prunes ownership records for terminals the listing no longer reports, so sustained churn in one workspace cannot evict a live terminal's record elsewhere. Depth is still checked, so a depth-2 leaf still cannot start a terminal. (#626)
- **Dependencies moved to current patches** — `@anthropic-ai/claude-agent-sdk` 0.3.272, `ai` 7.0.101, `tailwind-merge` 3.7.0, and a dev group of eight carrying Vite 8.3.0, Vitest and `@vitest/coverage-istanbul` 5.0.1, Biome 2.5.13, happy-dom 20.14.5, i18next-cli 1.73.3, `@tanstack/router-plugin` 1.168.38, and `@testing-library/dom` 10.4.2. (#618, #619, #620, #622)
- **The guide is pinned to v0.1.19 and the drift the release audit found is swept.** `master` carries no branch protection or ruleset, so `verify` is documented as unenforced rather than as a required check; the Concierge holds six Concierge-only ops, not four; the environment-variable catalogue has 26 entries including six reserved `ENSEMBLR_CONTROL_*` names, not 20; `[git]` has five snake_case keys besides `branch_prefix`; `NATIVE_MCP_PROVIDERS` is `MCP_CLIENT_RUNTIMES`, reached through `usesNativeControlMcp`, and `ControlAudience` is no longer three fields; and the Checks guidance records the fast-poll and not-ready-while-checking behaviour from #615. Install asset names and the update-feed example are copied off the published release. (#617)
- **TypeSafe AI's Jev is evaluated and recorded, not adopted.** `docs/considerations/jev-decision-layer.md` and `jev-spike-runbook.md` set out where a sidecar classifier could fit — spawn model and thinking-level selection behind a toggle, question triage, child-report flags, a Linear duplicate check, AFK decision ranking — and where it must never sit: permission gating, the Plan Mode guard, and frontier-model approval. The Phase 0 labelling spike and its correction are recorded with the run artifacts; `TYPESAFE_API_KEY` is added to `.env.example` for the local evaluation scripts. Nothing is wired into the app. (#624, #625)

### Fixed

- **A chat follows the newest message until you scroll away from it, and only then.** Three gaps in `use-stick-to-bottom` broke that. It observed the content element alone, so a composer that grew — a queued follow-up stacking up, the background-tasks notice appearing, a draft wrapping — shrank the viewport without the content changing height and slid the newest message under the composer while the lock still read as armed. Its own escape hatch gave up whenever `resizeDifference` was set, which a turn being written keeps set continuously, so reaching for PageUp or the scrollbar mid-stream pulled you straight back down. And an escape never expired, so a transcript abandoned mid-scroll stayed parked there however long the agent went on writing. `useConversationViewportResize` now watches the viewport itself and decides from geometry — the height it had a moment ago against an untouched `scrollHeight`/`scrollTop` — rather than from `escapedFromLock`, which the library never clears on a programmatic scroll; `useConversationScrollHold` reads the gestures directly, guarded by the near-bottom threshold so a shrinking turn's clamp is not mistaken for one; and `useConversationIdleFollow` retires an escape after 60s with no input anywhere in the window, deferring while a text selection is live. (#628)
- **Archiving a workspace that adopted an existing branch no longer deletes that branch.** `branchCleanup` was trusted straight from the archive request and never checked against the `adoptedBranch` field creation records, so with `delete_local_branch_on_archive` enabled the archive force-deleted a branch the app never cut and skipped `reclaimWorktreeDisk` — the path the recovery snapshot lives on — taking the branch and its commits with it. `resolveBranchCleanup` now clears the flag for an adopted workspace in the main process, where the renderer setting cannot route around it, and the guard covers the continuation chain so `deleteBranchChain` cannot reach an adopted predecessor. (#627)
- **A turn checkpoint no longer deletes a tracked file that later gained an ignore rule.** `writeWorkingTree` staged into a fresh, empty temporary index, and `git add -A` skips any path an ignore rule matches, so such a file was absent from the snapshot tree — and restoring that snapshot ran `read-tree -u --reset` against the real index, which still tracked it, and removed it from disk. The temporary index is now seeded from HEAD before the overlay, so every tracked path stays in the tree while modifications and deletions still apply; an unborn branch has no HEAD tree and skips the seed. `diffTrees` also passes `-c core.quotePath=false`, so a non-ASCII path stops arriving C-quoted with octal escapes in all three sections of the combined diff. (#627)
- **Clearing a Linear assignee, project, or cycle now clears it.** Picking Unassigned, No project, or No cycle set the field to `UNSET_FIELD`, which `buildUpdateIssueRequest` dropped — Linear kept the old value, and when the clear was the only edit the builder returned null and nothing was sent at all. `dueDate` worked only because it was the one field typed `| null` end to end; `assigneeId`, `projectId`, and `cycleId` are widened the same way across the IPC contract, the Zod schema, and the client, so an absent key still means leave alone and a null means clear. Priority was never affected. (#627)
- **A Review model pin survives hiding that model.** `reviewModelRow` resolved the pin through `listModels`, which filters hidden models, so hiding a pinned Review model silently returned null and `openReview` fell back to the caller's own model and runtime without saying so. An `includeHidden` option is threaded through `listModelsFor`, `resolveForSpawn`, the conversation port, and its adapter; it defaults off everywhere and the only caller that sets it is the internal Review spawn, gated on a pin having resolved. `brief.model` comes from the renderer's Review settings rather than from an agent, so a delegating agent still cannot reach a hidden model by naming one. (#627)
- **A composer holding only attachments is no longer emptied by an auto-submit.** `deliverPrimedAction` tested `readDraft().text.trim()`, but the linearizer drops a chip's stand-in space, so an attachment-only draft read as empty, `submitText` ran, and the attachments went with the clear. It now uses the module's own `isEmptyDraft`, which already recognises attachment segments. Separately, `useChangesSource` discarded the branch-scoped query's error and substituted working-tree rows, so a bad or deleted base ref made **All changes** show uncommitted edits — or nothing — and report success; a branch comparison that errors now surfaces the error and shows no rows, with the substitution kept for the two cases it serves. (#627)
- **An oversize text attachment is announced by path instead of failing the send.** The attachment store accepts files up to 50 MiB but the workspace file read refuses anything over 512 KiB with `too-large`, so a pasted or @-mentioned text file past that cap made the whole send throw. Such a file now serializes as a path-only `<attached_file>` placeholder carrying its size, so the agent opens the file itself. Every other read error still fails the send. (#623)
- **A diff payload that will not parse shows the patch it still carries.** `presentWorkspaceDiff` returned an empty body whenever `workspaceDiffData` came back null, leaving nothing but the collapsed raw disclosure — reachable in ordinary use, because `MAX_AGENT_PAYLOAD_CHARS` truncates the MCP transport's text and truncated JSON does not parse. The fallback now recovers the `diff` string of a cut-off envelope, decoded as far as the escapes are intact, or a bare patch, and renders it as a diff; any other non-blank text renders as a code body, and only genuinely blank output stays empty. New-file, deleted-file, rename, and multi-file patches gain the test coverage the ticket's acceptance criteria had never had. (#627)
- **Two suites stop failing on a loaded runner.** The `linux-recursive-watch` suite is gated to Linux — `defaultStartWatch` reaches `startLinuxRecursiveWatch` only when `process.platform === 'linux'`, so running it on macOS asserted a path production never takes there against a wall clock sized for inotify. The stubborn-descendant test gets a 10s RPC budget and an explicit 30s Vitest timeout, because reaching `extension_ui_request` takes two real node boots and the generic RPC timeout was answering instead of the interaction diagnosis the test asserts; the happy path is untouched and the file still runs in ~2s. (#627)

### Security

- **Plan Mode denies BSD `date`'s bare clock-setting operand.** The guard screened `date -s`/`--set` through `FLAG_GUARDED_COMMANDS`, but BSD `date` also reaches `clock_settime` from a positional with no flag at all (`date 010100002026`), which classified as read-only and was allowed in Plan Mode and for the Concierge. A positional evaluator now runs after the flag guard, so the flag denial keeps precedence: only a `+FORMAT` operand reads, BSD's `-j` disarms the check, and value-taking letters are skipped in both the attached and separate spellings. (#627)
- **Disconnecting a Linear account can no longer be raced into keeping its credentials.** `disconnect()` deletes the account and drops the in-flight map entry but cannot cancel a `refreshAccessToken` exchange already running; that exchange wrote its new tokens under the deleted account id, and `getAccessToken` read the missing account's expiry as null — which `isExpired` reports as not expired — and handed the orphan credential back as valid. The refresh now re-checks the account before and after the write and discards tokens that arrive for one that has vanished, `getAccessToken` treats a missing row as not-connected, and `writeTokens` refuses an unknown account id with a typed `LinearAuthError` so the refusal is not recorded as a failing refresh. `LinearAccountStore.delete` also removes the row before the secrets, so a refresh resuming mid-disconnect sees no account and discards what it fetched. This stops Ensemblr retaining and serving the credential locally; it does not revoke the grant with Linear. (#627)
...
- **Per-turn diff on the transcript footer and a "Latest turn" source in the Changes panel.** A turn's footer now carries the wall-clock time it ended and a chip per file it changed, with that file's line counts; clicking a chip opens the file at the turn's scope in the normal diff viewer. The Changes panel gains a "Latest turn" source resolved from the workspace's newest checkpoint. Both surfaces route through one `{ kind: 'turn', fromRef, toRef? }` scope on the diff plumbing — `fromRef` is the turn's pre-prompt checkpoint (ADR 0012), an absent `toRef` means the live working tree. The transcript is not virtualized, so each turn's `git diff` is held back until its footer scrolls into view. (#592)
- **Structured bodies for the app's own control-tool timeline rows.** Every `ensemblr_*` row but one used to unfold onto a raw JSON dump — a wait on delegated children rendered as one unwrapped line. Twelve ops now get real bodies: `wait_for_agents`, `read_conversation`, `get_last_message`, `get_conversation_status`, `list_models`, `recall_memory`, `list_workspaces`, `list_terminals`, `read_terminal_output`, `linear_list_issues`, `linear_get_issue`, `get_diff_comments`, plus `resolve_diff_comments` (which shows the ids it could not close). The rest correctly get an empty body under a "Raw execution" disclosure. Dispatch now resolves control tools by canonical name across the two runtimes rather than by the surface name they reach the timeline under. (#600)
- **A ready-to-merge PR with unpushed commits is flagged rather than reported green.** The sidebar row and the board card now show `pr-unpushed` — the same up-arrow the header's Push action uses, in the warning tone — when GitHub's mergeable verdict is running against a tip the local branch has moved past. The cached PR snapshot's `branchSync` reaches the renderer, so each row builds its real git-status row from it with no extra git query, and the open workspace's row folds in uncommitted edits from the working-tree status the route layout already reads. (#601)
- **A sub-agent's tab now withholds its close control while its delegate is running,** so a mouse close or ⌘W cannot stop the agent the orchestrator is still waiting on. The refusal lands in `resolveRunningCloseTarget`, which the tab strip and the ⌘W handler already share, and unions the live composer flag with the persisted snapshot so ⌘W is never weaker than the affordance. A finished delegate whose orchestrator never cleaned up still closes straight through. (#599)
- **Claude Code background tasks read distinctly across the app.** `Bash` with `run_in_background`, an `Agent` with `async_launched`, and the poll/stop verbs (`TaskOutput`/`BashOutput`, `TaskStop`/`KillShell`/`KillBash`) now render as their own timeline rows with the task id in the title; foreground `Bash` gains an outcome suffix. A blue sibling to the terminal-activity dot lights on the sidebar row and the dashboard card while any background task is live, driven by the runtime's own `background_tasks_changed` level signal reduced with replace semantics. A composer notice names what this chat left running, the quit confirmation counts background tasks alongside mid-turn chats, and the close-chat guard defers an idle tab that still holds tasks. Read-only mode now withholds the modern task verbs alongside the legacy aliases. (#604)

### Changed

- **The three turn-diff display toggles hoist into one panel header.** Unified/split, hidden characters, and word wrap mutate the same atoms, so a stack of five files used to carry five copies of one control acting on all of them at once. `DiffToolbar` splits: `DiffDisplayToggles` is the zero-prop component holding the three toggles, and `DiffViewer` gains a `showToolbar` opt-out so a surface stacking many viewers can suppress the per-file toolbar. `TurnDiffPanel` hoists one copy into its own header beside the file count. (#605)

### Fixed

- **A Linear issue's chip appears immediately on pick, comments fill in behind it.** Picking a Linear issue from the composer's "Link issue" picker used to dismiss the dialog and then show nothing for about five seconds — one blocking GraphQL round-trip on every first pick, because the main-process comment cache never wrote `commentsSyncedAt` from list sync so its staleness gate never passed. `attachLinearIssue` now runs in two passes: the chip lands from the row the picker already holds, and the comments are fetched behind it and the chip repointed at the fuller document. GitHub issues arrive whole from `gh` and take the first pass only. `AttachmentNode.setAttachment` behind `ComposerEditorHandle.updateAttachment` carries the swap under `HISTORY_MERGE_TAG`, so the repoint neither costs an undo nor forgets the state it produced. (#598)
- **The file search dialog ranks its results here instead of leaving it to cmdk.** Rendering every previewable workspace file — up to the 5,000-entry ceiling — behind cmdk made typing jerky, opened a filtered set mid-list, and could leave Enter inert when the row holding the selection unmounted mid-keystroke. Ranking happens behind `shouldFilter={false}` with the existing fuzzy scorer, debounced at 80ms, list rewound on a settled query, and the selection held explicitly across re-ranks. Search state moves into a child the dialog mounts, so each ⌘P opens on an empty box. `foldForMatching` was 63% of a 25-87ms per-keystroke pass; pure ASCII now takes a native whole-string `toLowerCase` (provably identical), non-ASCII keeps the character walk to preserve contextual rules. Full ranking pass: 3-6.5ms. (#597)
- **The "Raw execution" disclosure stays inside its tool row instead of painting over the next two.** `MessageResponse` put `size-full` on the Streamdown root, so a flex-item chain measured markdown plus `mt-3` plus the details for the parent's height, then let the markdown's `height: 100%` expand to it and push the details past the parent's bottom edge. Latent until #600 gave every control row a raw-execution disclosure; the same swallow hit `ChatMessageText`. Sized by width only now. (#602)
