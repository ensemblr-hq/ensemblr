...
export default function ensemblrControl(pi: ExtensionAPI): void {
	...
	tool(
		...
		'startReview',
		"Open this workspace's Review conversation over the change you have made — the same review the user's Review button runs, deferring to whatever review skill this repository ships, carrying the user's own review instructions, on the model they picked for reviews. Call it only when the user explicitly asks for an agent review. AFK alone is not a request. Do not start it as routine verification; review your own diff or choose a reviewer through ordinary delegation under your role's rules. The configured Review model does not pin those delegates. What it opens is a root orchestrator with its own delegation budget, not your child, so it can spawn its own readers over a wide diff — which also means ensemblr_wait_for_agents will not pick it up by default and you must name its agentSessionId in `targets`. It shares this worktree with you: leave the files alone while it works. When it reports, send its findings back to the SAME conversation with ensemblr_send_follow_up and have it fix them there; you stay the committer and you own the pull request. Pass a short `title` when this is not the workspace's only review. Calling it again while that reviewer still exists hands the same one back rather than opening a second, and its tab is the user's record of the review rather than a scratch tab of yours to close. It takes one of the workspace's co-tenancy slots — two agents may write one checkout while the user is here, four while they are away — so a workspace already at that limit refuses it.",
		Type.Object({ title: Type.Optional(Type.String()) }),
	);
	tool(
	...
	tool(
		...
		'listModels',
		"List the models you can spawn a child on (id, runtime, vendor, display name, thinking ladder, cost tier) plus the default. This is the live source of permitted destinations: it returns each model's runtime and role preferences plus the caller runtime and allowed runtime policy. Choose only an id from a reported row, whose runtime must appear in `allowedRuntimes`; any other destination is refused, and a cross-runtime destination is used only when that policy allows it. A terminal caller receives every runtime because the app cannot identify its runtime; its `model` remains mandatory. `thinkingLevels` is the ladder that model accepts and `thinkingAxis` names what its runtime calls the dial (`effort` on Claude Code, `thinking` on pi) — pass one of those as `thinkingLevel`. `tier` is `frontier` for the costliest models: spawning a child on one is put to the user for confirmation, so prefer a `standard` id unless the task genuinely needs more.",
		empty,
	);
	tool(
